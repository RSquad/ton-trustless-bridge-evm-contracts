...
contract TreeOfCellsAdapter {
        function get_tree_of_cells(bytes calldata boc, BagOfCellsInfo memory info) public pure returns (CellData[50] memory cells) {
    ...
    function deserialize_cell(uint idx, bytes calldata cells_slice, uint[50] memory custom_index, uint ref_byte_size, uint cell_count) public pure returns (CellData memory cell) {
